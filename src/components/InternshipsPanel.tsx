...
export default function InternshipsPanel({
  ...
  const [securityFilter, setSecurityFilter] = useState<"all" | "safe">("safe");
  ...
  const filteredInternships = internships
    .filter(item => {
      ...
      // Safety scam filter
      if (securityFilter === "safe" && item.scamFlag) {
        return false;
      }
      // Field filter
...
<button
      ...
      <div className="bg-holo-gray-dark border border-holo-gray-border p-4 grid grid-cols-1 md:grid-cols-5 gap-4">
        ...
        {/* Audit Filter (Scams review) */}
        <div className="space-y-1.5">
          <label className="text-xs font-mono text-holo-blue-light block uppercase">Audit Safe Mode</label>
          <div className="flex gap-2">
            <button
              onClick={() => setSecurityFilter("safe")}
              className={`flex-1 text-xs font-mono px-3 py-1.5 border transition-all duration-150 cursor-pointer text-center uppercase active:scale-95 ${securityFilter === "safe" ? "bg-holo-blue-dark text-black border-holo-blue-light" : "bg-black text-gray-400 border-holo-gray-border hover:brightness-110"}`}
            >
              Verified Listings
            </button>
            <button
              onClick={() => setSecurityFilter("all")}
              className={`flex-1 text-xs font-mono px-3 py-1.5 border transition-all duration-150 cursor-pointer text-center uppercase active:scale-95 ${securityFilter === "all" ? "bg-red-950/60 text-red-300 border-red-700/60" : "bg-black text-gray-400 border-holo-gray-border hover:brightness-110"}`}
              title="Includes suspected scam simulations for testing filters."
            >
              Flagged Scams
            </button>
          </div>
            ...
            return (
              <div
                ...
                className={`bg-holo-gray-dark border p-4 flex flex-col md:flex-row gap-4 items-start md:items-stretch transition-all duration-200 hover:scale-[1.01] ${
                  item.scamFlag
                    ? "border-amber-700/60 bg-gradient-to-r from-red-950/10 to-amber-950/20"
                    : item.isNew
              ...
              >
                ...
                <div className="flex-grow space-y-2">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <h4 className="text-base font-bold font-sans text-gray-100 flex items-center gap-1.5 mt-0.5">
                        {item.title}
                        {item.isVerified && !item.scamFlag && (
                          <div className="relative group/tooltip inline-block leading-none">
                            <CheckCircle className="w-4 h-4 text-holo-blue-light fill-black cursor-help" />
                            <span className="absolute bottom-full left-1/2 -translate-x-1/2 mb-1.5 hidden group-hover/tooltip:block bg-holo-gray-light text-white text-3xs font-mono px-2 py-1 border border-holo-gray-border whitespace-nowrap rounded-none pointer-events-none z-10 shadow-lg leading-none">
                              Verified Safe Employer
                            </span>
                          </div>
                        )}
                      </h4>
                  ...
                  {/* SCAM BANNER if flagged */}
                  {item.scamFlag && (
                    <div className="bg-red-950/40 border border-amber-800 p-2.5 text-3xs font-mono text-amber-300 leading-normal flex items-start gap-2">
                      <AlertTriangle className="w-4 h-4 shrink-0 text-amber-500" />
                      <div>
                        <strong>SUSPECTED EMPLOYMENT FRAUD:</strong> {item.scamReason}
                      </div>
                    </div>
                  )}
                </div>
